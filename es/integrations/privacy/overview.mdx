...
Si necesitas comprobar si un usuario ya ha dado su consentimiento a las cookies para el cumplimiento del RGPD, puedes especificar una key y un valor en el almacenamiento local en `cookies`:
...
Si se establecen estos valores, se comprobará el almacenamiento local para verificar si el usuario ha dado su consentimiento a las cookies. Si no lo ha hecho, se desactivará telemetry.

Si deseas desactivar telemetry para todos los usuarios, puedes agregar lo siguiente a tu archivo `docs.json`:
