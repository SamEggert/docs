...
<Warning>Mintlify trata `oneOf` y `anyOf` de la misma manera, ya que la diferencia práctica rara vez afecta el uso de la API.</Warning>
...
Cuando usas `allOf`, Mintlify realiza cierto preprocesamiento de tu documento OpenAPI para mostrar combinaciones complejas de forma legible. Por ejemplo, al combinar dos esquemas de objeto con `allOf`, Mintlify une las propiedades de ambos en un único objeto. Esto es especialmente útil al aprovechar los [components](https://swagger.io/docs/specification/components/) reutilizables de OpenAPI.
...
<ParamField body="org_with_users" type="object">
  <Expandable>
    ...
    <ParamField body="users" type="object[]">
      Una matriz que incluye a todos los usuarios de la organización
    </ParamField>
  </Expandable>
</ParamField>
...
Cuando usas `oneOf` o `anyOf`, las opciones se muestran en un contenedor con pestañas. Especifica un campo `title` en cada subesquema para asignar nombres a las opciones. Por ejemplo, así podrías mostrar dos tipos distintos de direcciones de entrega:

```yaml
delivery_address:
  oneOf:
    - title: DirecciónCalle
      type: object
      properties:
        address_line_1:
          type: string
