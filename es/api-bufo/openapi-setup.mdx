...
description: "Hace referencia a endpoints de OpenAPI en tus páginas de documentación"
---
...
* [Swagger&#39;s OpenAPI Guide](https://swagger.io/docs/specification/v3_0/basic-structure/) para aprender la sintaxis de OpenAPI.
* [The OpenAPI specification Markdown sources](https://github.com/OAI/OpenAPI-Specification/blob/main/versions/) para consultar los detalles de la especificación OpenAPI más reciente.
* [Swagger Editor](https://editor.swagger.io/) para editar, validar y depurar tu documento OpenAPI.
* [The Mint CLI](https://www.npmjs.com/package/mint) para validar tu documento OpenAPI con el comando: `mint openapi-check <openapiFilenameOrUrl>`.
...
Para habilitar funciones de Mintlify como el API playground, agrega un campo `servers` a tu documento OpenAPI con la URL base de tu API.
...
El API playground utiliza estas URL de servidor para determinar adónde enviar las solicitudes. Si especificas varios servidores, un menú desplegable permitirá a los usuarios cambiar entre ellos. Si no especificas un servidor, el API playground usará el modo simple, ya que no puede enviar solicitudes sin una URL base.

Si tu API tiene endpoints que existen en distintas URL, puedes [sobrescribir el campo servers](https://swagger.io/docs/specification/v3_0/api-host-and-base-path/#overriding-servers) para una ruta u operación determinada.

<div id="specifying-authentication">
  ### Especificar la autenticación
</div>

Para habilitar la autenticación en la documentación de tu API y en el API playground, configura los campos `securitySchemes` y `security` en tu documento de OpenAPI. Las descripciones de la API y el API Playground agregarán campos de autenticación según la configuración de seguridad en tu documento de OpenAPI.
...
<Steps>
  ...
  <Step title="Aplica la autenticación a tus endpoints.">
    Agrega el campo `security` para exigir autenticación.
...
* [API Keys](https://swagger.io/docs/specification/authentication/api-keys/): Para claves en header, query o cookie.
...
La extensión `content` es compatible con todos los components y el formato MDX de Mintlify.
...
Cuando `x-mint: href` está presente, la entrada de navegación enlazará directamente a la URL especificada en lugar de generar una página de la API.
...
<div id="auto-populate-api-pages">
  ## Autogenerar páginas de la API
</div>

Agrega un campo `openapi` a cualquier elemento de navegación en tu `docs.json` para generar automáticamente páginas para endpoints de OpenAPI. Puedes controlar dónde aparecen estas páginas en la estructura de navegación, ya sea como secciones de API dedicadas o junto con otras páginas.
...
Las páginas de endpoint generadas tienen estos metadatos predeterminados:
...
Genera secciones de API dedicadas añadiendo un campo `openapi` a un elemento de navegación y ninguna otra página. Todos los endpoints de la especificación se incluirán:
...
Cuando necesites más control sobre dónde aparecen los endpoints en tu documentación, puedes referenciar endpoints específicos en la navegación. Este enfoque te permite generar páginas para endpoints de la API junto con otros contenidos.
...
Cualquier entrada de página que coincida con el formato `METHOD /path` generará una página de la API para ese endpoint usando la especificación OpenAPI predeterminada.

<div id="openapi-spec-inheritance">
  #### Herencia de la especificación de OpenAPI
</div>

Las especificaciones de OpenAPI se heredan a lo largo de la jerarquía de navegación. Los elementos de navegación hijos heredan la especificación de OpenAPI de su elemento principal, a menos que definan la suya propia:
...
Este enfoque es útil cuando necesitas endpoints individuales de distintas especificaciones o solo quieres incluir algunos endpoints específicos.
...
Cuando haces referencia a una operación de OpenAPI de esta manera, el nombre, la descripción, los parámetros, las respuestas y el API playground se generan automáticamente a partir de tu documento de OpenAPI.

Si tienes varios archivos de OpenAPI, incluye la ruta del archivo en la referencia para asegurarte de que Mintlify encuentre el documento de OpenAPI correcto. Si solo tienes un archivo de OpenAPI, Mintlify lo detectará automáticamente.
...
Usa nuestro [scraper](https://www.npmjs.com/package/@mintlify/scraping) de Mintlify para autogenerar páginas `MDX` para documentos OpenAPI grandes.
...
<Steps>
  ...
  <Step title="Especificar una carpeta de salida.">
    ...
    Agrega la opción `-o` para indicar una carpeta donde guardar los archivos. Si no se especifica una carpeta, los archivos se guardarán en el directorio de trabajo.
  </Step>
</Steps>
...
<Note>
  El nombre del webhook debe coincidir exactamente con la key definida en el
  campo `webhooks` de tu especificación de OpenAPI.
</Note>
