...
- `/.well-known/vercel/*` - Requerido para la verificación del domain
...
<Frame>
    ![Página de CloudFront "Create distribution" que muestra "acme.mintlify.dev" como el Origin domain.](/images/cloudfront/origin-name.png)
  </Frame>
...
2. Busca la URL de staging que refleje el domain principal. Esto puede variar mucho según cómo se aloje tu landing page. Por ejemplo, la URL de staging de Mintlify es [mintlify-landing-page.vercel.app](https://mintlify-landing-page.vercel.app).

<Info>
  Si tu landing page está alojada en Webflow, usa la URL de staging de Webflow. Se verá como `.webflow.io`.

  Si usas Vercel, utiliza el domain `.vercel.app` disponible para cada proyecto.
</Info>
...
Para este punto, deberías tener dos Origins: uno con `[SUBDOMAIN].mintlify.app` y otro con tu URL de staging.
...
Los comportamientos en CloudFront permiten controlar la lógica de subrutas. En términos generales, queremos implementar la siguiente lógica:

- **Si un usuario llega a tu subruta personalizada**, dirígelo a `[SUBDOMAIN].mintlify.dev`.
- **Si un usuario llega a cualquier otra página**, dirígelo a la página de destino actual.
...
Crea comportamientos para las rutas de verificación de dominio de Vercel con un **Patrón de ruta** de `/.well-known/*` y establece **Origen y grupos de origen** a la URL de tu documentación.

Para "Política de caché", selecciona **CachingDisabled** para garantizar que estas solicitudes de verificación pasen sin almacenamiento en caché.
...
<Info>
Si `.well-known/*` es demasiado genérico, puede limitarse, como mínimo para Vercel, a 2 comportamientos:
...
<div id="check-behaviors-are-set-up-correctly">
  ### Verifica que los comportamientos de Check estén configurados correctamente
</div>
...
Todas las páginas deberían dirigir a tu página de inicio principal, pero si añades el subpath que elegiste, por ejemplo `/docs`, a la URL, deberías ver que te lleva a tu instancia de documentación de Mintlify.
...
<Note>
  Para esta sección, también puedes consultar la guía oficial de AWS: [Configurar
...
4. Activa `Alias` y, en **Route traffic to**, elige la opción `Alias to CloudFront distribution`.
...
Tu documentación ya está activa en la subruta que elegiste de tu dominio principal.
